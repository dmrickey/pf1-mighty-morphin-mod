...
class DirectoryPicker extends FilePicker {
    ...
    static async forgeCreateDirectory(target) {
        ...
        if (!response || response.error) {
            throw new Error(response?.error || 'Unknown error while creating directory.');
        }
    }
            ...
            for (let i = 0; i < paths.length; i += 1) {
                try {
                    ...
                    if (!err.startsWith('EEXIST') && !err.startsWith('The S3 key')) {
                        console.log(`Error trying to verify path [${parsedPath.activeSource}], ${parsedPath.current}`, err);
                    }
                }
            }
        }
