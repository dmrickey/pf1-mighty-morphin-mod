...
export class MorphinBeastShape extends MorphinPolymorphDialog {
    ...
    async getData() {
        ...
        formData.animalOptions = this.shapeOptions.animal.filter(o => o.size === defaultSize);

        // Create radio button data for animal sizes, set the one for the default size as the default checked button
        formData.animalSizes = this.sizes.animal.map(o => { return o === defaultSize ? { label: o, size: CONFIG.PF1.actorSizes[o], default: true } : { label: o, size: CONFIG.PF1.actorSizes[o] }; });
        formData.mBeastSizes = this.sizes.magicalBeast;
        ...
        formData.defaultChoice = formData.animalOptions[0];
        formData.previewHtml = await this.buildPreviewTemplate(formData.defaultChoice.name, 'animal');
    ...
    async buildPreviewTemplate(chosenForm, chosenType) {
        ...
        for (let i = 0; i < this.changes.length; i++) {
            ...
            else if (!change.target && change.subTarget == 'nac') {
                // natural AC change
                if (templateData.scoreChanges.length > 0) {
                    templateData.scoreChanges += ', ';
                }
        ...
        for (let i = 0; i < Object.keys(this.speeds).length; i++) {
            ...
            templateData.speedChanges += `${speedName} ${this.speeds[speedName]} ft`;
        }
        ...
        for (let i = 0; i < specialAttackList.length; i++) {
            ...
            for (let j = 0; j < (specialAttack.special?.length || 0); j++) {
                ...
                if (!MorphinBeastShape.allowedSpecials[this.level].includes(special)) {
                    valid = false;
                }
            }
            ...
            if (valid) {
                ...
                if (specialAttack.nonCrit) {
                    damageDice += (!!damageDice.length ? ' plus ' : '') + `${specialAttack.nonCrit[0]} ${specialAttack.nonCrit[1]}`;
                }
        ...
        for (let i = 0; i < this.senses.length; i++) {
            ...
            if (!!senseEnumValue) {
                ...
                templateData.senses += `${MorphinChanges.SENSES[Object.keys(MorphinChanges.SENSES)[senseEnumValue - 1]].name}`; // enum value 1 = SENSES[0] = LOWLIGHT
            }
        }
        if (!templateData.senses.length) {
            templateData.senses = 'None';
        }
        ...
        let newHtml = `${!!templateData.polymorphBase ? '<p><span class="previewLabel">Base Size Adjust: </span><span id="polymorphScores">' + templateData.polymorphBase + '</span></p>' : ''}
            ...
            ${this.level === 4 ? '<p><span class="previewLabel">Energy Resistances: </span><span id="eres">' + templateData.eres + '</span></p>' +
